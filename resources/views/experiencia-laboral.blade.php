                ...
                <div class="form-row">
                    ...
                    <div class="form-group">
                        ...
                        <div class="date-picker-wrap">
                            {{-- FIX: oninput + onchange para capturar escritura manual --}}
                            <input type="date" id="fechaInicioPicker" class="form-input date-picker"
                                min="1950-01-01" max="{{ date('Y-m-d') }}"
                                value="{{ old('fecha_inicio_anio') && old('fecha_inicio_mes') && old('fecha_inicio_dia')
                                    ? old('fecha_inicio_anio').'-'.str_pad(old('fecha_inicio_mes'),2,'0',STR_PAD_LEFT).'-'.str_pad(old('fecha_inicio_dia'),2,'0',STR_PAD_LEFT)
                                    : (old('fecha_inicio') ?? '') }}"
                                onchange="syncFechaInicio(this.value)"
                                oninput="syncFechaInicio(this.value)">
                        </div>
                        <input type="hidden" name="fecha_inicio"      id="fechaInicioFull" value="{{ old('fecha_inicio') }}">
                    ...
                    <div class="form-group" id="fechaFinGroup">
                        ...
                        <div class="date-picker-wrap">
                            ...
                            <input type="date" id="fechaFinPicker" class="form-input date-picker"
                                min="1950-01-01" max="{{ date('Y-m-d') }}"
                                value="{{ old('fecha_fin_anio') && old('fecha_fin_mes') && old('fecha_fin_dia')
                                    ? old('fecha_fin_anio').'-'.str_pad(old('fecha_fin_mes'),2,'0',STR_PAD_LEFT).'-'.str_pad(old('fecha_fin_dia'),2,'0',STR_PAD_LEFT)
                                    : (old('fecha_fin') ?? '') }}"
                                onchange="syncFechaFin(this.value)"
                                oninput="syncFechaFin(this.value)">
                        </div>
                        <input type="hidden" name="fecha_fin"      id="fechaFinFull" value="{{ old('fecha_fin') }}">
...
@push('scripts')
    ...
    {{-- FIX: listener de Quill en capture=true (corre antes de validación)
         Solo sincroniza el hidden — NO llama preventDefault ni stopPropagation --}}
    <script>
    (function () {
        const TOOLBAR = [
            [{ font: [] }],
        ...
        /* capture=true: sincroniza Quill ANTES de que corra la validación general.
           No interfiere con preventDefault porque no lo llama. */
        document.getElementById('experienciaForm').addEventListener('submit', function () {
            const html = window.quillAdd.root.innerHTML;
            hiddenEl.value = (html === '<p><br></p>') ? '' : html;
        }, true);
