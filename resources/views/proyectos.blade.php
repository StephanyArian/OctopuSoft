...
<x-app-layout>
    ...
    (function() {
        ...
        // ── PROYECTOS ─────────────────────────────────────────────
        function cargar() {
            try { proyectos = JSON.parse(localStorage.getItem(STORAGE_KEY) || '[]'); }
            catch(e) { proyectos = []; }
            renderizar();
        }

        function guardar() { localStorage.setItem(STORAGE_KEY, JSON.stringify(proyectos)); }
        ...
        function mostrarForm() { formCard.classList.add('open'); inputNombre.focus(); }
        ...
        function renderizar() {
            ...
            document.querySelectorAll('.btn-eliminar').forEach(b =>
                b.addEventListener('click', () => eliminarProyecto(parseInt(b.dataset.id))));
        }
        ...
        function guardarProyecto() {
            ...
            if (editandoId) {
                const idx = proyectos.findIndex(p => p.id === editandoId);
                proyectos[idx] = { ...proyectos[idx], nombre, descripcion, fecha, estado, tecnologias };
                window._proyectoParaEvidencias = proyectos[idx];
            ...
            guardar(); ocultarForm(); renderizar();
        }
        ...
        function mostrarToast(mensaje, tipo) {
            ...
            t.className = 'proy-toast' + (tipo === 'error' ? ' error' : '');
            t.innerHTML = mensaje;
