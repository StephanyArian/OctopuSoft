@forelse($portfolios as $portfolio)
    ...
    $technicalSkillsCount = $portfolio->user->skills
        ->where('type', 'technical')
        ->where('is_visible', true)
        ->count();

    $visibleProjectsCount = $portfolio->projects
        ->where('is_visible', true)
        ->count();

    $workExperiencesCount = $portfolio->user->experiences
        ->where('type', 'work')
        ->where('is_visible', true)
        ->count();

    $educationCount = $portfolio->user->experiences
        ->where('type', 'education')
        ->where('is_visible', true)
        ->count();

    $languagesCount = $portfolio->user->skills
        ->where('type', 'language')
        ->where('is_visible', true)
        ->count();

    $networksCount = $portfolio->user->professionalNetworks
        ->where('is_visible', true)
        ->count();
    ...
    $portfolioPercent = min(100,
        (filled($portfolio->user->photo_base64) ? 8 : 0) +
        (filled($portfolio->user->biography) ? 12 : 0) +
        (filled($portfolio->user->profession_id) ? 10 : 0) +
        (min($technicalSkillsCount, 5) * 4) +
        (min($visibleProjectsCount, 4) * 5) +
        (min($workExperiencesCount, 3) * 4) +
        (min($educationCount, 2) * 4) +
        (min($languagesCount, 3) * 2) +
        (min($networksCount, 2) * 2)
    );
@endphp
...
{{-- Barra de completitud + botón ver portafolio --}}
<div class="card-footer-row">
    <div class="portfolio-rating" title="Portafolio {{ $completedSections }}/9 secciones completas">
        <div class="rating-meta">
            <span class="rating-label">Completitud</span>
            <span class="rating-text">{{ $portfolioPercent }}%</span>
        </div>

        <div class="rating-bar">
            <span class="rating-bar-fill" style="width: {{ $portfolioPercent }}%;"></span>
        </div>
    </div>
